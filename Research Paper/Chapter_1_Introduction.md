...
4. **Doctor Portal Development**: Implement a comprehensive doctor portal that enables medical practitioners to manage their availability, set consultation time slots with 15-minute duration granularity, manage appointments, and access patient information and consultation history. Access to the doctor portal is gated behind an administrative approval workflow: newly registered doctors must submit their medical licence number and degree document for admin review, and only approved doctors are permitted to log in and access the dashboard. This credential verification mechanism ensures that only qualified, verified practitioners are listed on the platform.
...
10. **Digital Prescription Management**: Implement a complete prescription workflow enabling doctors to issue structured digital prescriptions (diagnosis, medicines with dosage/frequency/duration, vital signs, advice, follow-up date) for completed consultations, and patients to view and retain their prescriptions in-app. The system must enforce one prescription per appointment (idempotency) and automatically notify patients upon prescription issuance.
...
**Gap 5: Absence of Persistent Medical Records and Digital Prescriptions**
Existing platforms do not automatically generate and maintain medical records from consultations, nor do they support structured digital prescriptions linked to specific consultations. MediCare addresses both gaps: the automated meeting notes feature creates AI-generated, structured transcripts and summaries of consultations, while the prescription management module allows doctors to issue formal digital prescriptions (with medicines, dosages, vital signs, and follow-up instructions) immediately after consultation completion. Both records are accessible to patients in-app and support continuity of care for follow-up consultations.

**Gap 6: No Doctor Identity and Credential Verification in Telemedicine Platforms**
Most telemedicine platforms in developing markets allow any self-declared practitioner to register and immediately begin accepting patient appointments, with no credential verification step. This creates significant patient safety risks: unqualified individuals may misrepresent themselves as licensed doctors, issue incorrect medical advice, and cause harm. MediCare addresses this critical gap through a mandatory administrative approval workflow: every new doctor registration is held in a pending state until an administrator reviews the submitted medical licence number and degree document, verifies credentials against authoritative licensing registries, and explicitly approves the account. Rejected registrations receive formal notification. Only verified, approved doctors appear in patient search results and are permitted to conduct consultations, ensuring that patients always interact with credentialed, qualified healthcare professionals.
...
**Backend Service (Node.js + Express + MongoDB)**: A RESTful API server managing user authentication, appointment scheduling, doctor profile management, video consultation coordination, and digital prescription management. The backend implements robust business logic for conflict-free appointment booking, real-time availability synchronization, prescription creation with idempotency enforcement, patient notification dispatch, and session management for both doctors and patients. A dedicated doctor approval module manages credential document ingestion (licence number and degree file via Multer), tri-state approval status tracking (pending/approved/rejected), login-time gating, and admin-facing approval and rejection endpoints — ensuring the platform maintains a verified-only pool of practicing doctors.
