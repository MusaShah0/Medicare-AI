...
const MyAppointments = () => {
  ...
  const [prescriptionViewData, setPrescriptionViewData] = useState(null);
  const [prescriptionViewLoading, setPrescriptionViewLoading] = useState(false);
  ...
  // ── PRESCRIPTION VIEW ──
  const openPrescriptionView = async (appointmentId) => {
    setPrescriptionView(appointmentId);
    setPrescriptionViewLoading(true);
    setPrescriptionViewData(null);
    try {
      const res = await axios.get(`${API_BASE}/prescription/${appointmentId}`, { withCredentials: true });
      if (res.data.success) {
        setPrescriptionViewData(res.data.data);
      }
    } catch {
      // no prescription — handled in modal
    } finally {
      setPrescriptionViewLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F4F7F9] font-sans text-slate-800">
      ...
      <nav className="bg-white border-b border-slate-100 shadow-sm sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <NotificationBell role="patient" light />
            <Link
              to="/doctors"
                    ...
                    <div className="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between gap-2 flex-wrap">
                      ...
                      <div className="flex items-center gap-2 flex-wrap justify-end">
                        ...
                        {joinable ? (
                          ...
                          <>
                            <button
                              onClick={() => openPrescriptionView(app._id)}
                              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-[#0A2540]/20 bg-white text-[#0A2540] text-xs font-bold hover:bg-[#0A2540]/5 transition"
                            >
                              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {prescriptionView && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm" onClick={() => { setPrescriptionView(null); setPrescriptionViewData(null); }} />
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto z-10">
            <div className="sticky top-0 bg-white border-b border-slate-100 px-8 py-5 flex items-center justify-between rounded-t-2xl z-10">
              <div>
                ...
                <h2 className="text-xl font-bold text-[#0A2540]">Your Consultation Prescription</h2>
              </div>
              <button
                onClick={() => { setPrescriptionView(null); setPrescriptionViewData(null); }}
            ...
            <div className="p-8">
              {prescriptionViewLoading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-bold text-[#0A2540] mb-2">No Prescription Yet</h3>
                  <p className="text-slate-400 text-sm">The doctor has not added a prescription for this appointment.</p>
                </div>
              ) : (
                <div className="space-y-6">
                  {/* Doctor info */}
                  {prescriptionViewData.doctor_id && (
                    <div className="bg-[#0A2540]/5 rounded-xl p-4">
                      ...
                      <p className="text-sm font-bold text-[#0A2540]">
                        Dr. {prescriptionViewData.doctor_id.first_Name} {prescriptionViewData.doctor_id.last_Name}
                      </p>
                      {prescriptionViewData.doctor_id.speciality && (
                        <p className="text-xs text-slate-400 mt-0.5">{prescriptionViewData.doctor_id.speciality}</p>
                      )}
                    </div>
                  )}

                  {/* Vital Signs */}
                  {(prescriptionViewData.vital_signs?.blood_pressure || prescriptionViewData.vital_signs?.temperature || prescriptionViewData.vital_signs?.pulse || prescriptionViewData.vital_signs?.weight) && (
                    <div>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Vital Signs</p>
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                        {[
                          { label: 'BP', val: prescriptionViewData.vital_signs.blood_pressure },
                          { label: 'Temp', val: prescriptionViewData.vital_signs.temperature },
                          { label: 'Pulse', val: prescriptionViewData.vital_signs.pulse },
                          { label: 'Weight', val: prescriptionViewData.vital_signs.weight },
                        ].filter(v => v.val).map(({ label, val }) => (
                          <div key={label} className="bg-[#F4F7F9] rounded-xl px-3 py-2.5 text-center">
                            <p className="text-[10px] text-slate-400 font-semibold">{label}</p>
                            <p className="text-sm font-bold text-[#0A2540] mt-0.5">{val}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {/* Diagnosis */}
                  <div>
                    <p className="text-[10px] font-bold text-[#00B4A0] uppercase tracking-widest mb-1">Diagnosis</p>
                    <p className="text-sm font-bold text-[#0A2540] bg-[#F4F7F9] rounded-xl p-3">
                      {prescriptionViewData.diagnosis || '—'}
                    </p>
                  </div>

                  {/* Medicines */}
                  {prescriptionViewData.medicines?.length > 0 && (
                    <div>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Medicines</p>
                      <div className="space-y-2">
                        {prescriptionViewData.medicines.map((med, i) => (
                          <div key={i} className="flex items-start gap-3 p-3 bg-[#F4F7F9] rounded-xl">
                  ...
                  {/* Advice */}
                  {prescriptionViewData.advice && (
                    <div>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Advice</p>
                      <p className="text-sm text-slate-700 bg-[#F4F7F9] rounded-xl p-3">{prescriptionViewData.advice}</p>
                    </div>
                  )}

                  {/* Follow-up date */}
                  {prescriptionViewData.follow_up_date && (
                    <div>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Follow-up</p>
                      <p className="text-sm font-bold text-[#0A2540] bg-[#F4F7F9] rounded-xl p-3">
                        {new Date(prescriptionViewData.follow_up_date).toLocaleDateString('en-US', {
                          weekday: 'long', year: 'numeric', month: 'long', day: 'numeric',
                        })}
                      </p>
                    </div>
                  ...
                  <button
                    onClick={() => { setPrescriptionView(null); setPrescriptionViewData(null); }}
                    className="w-full py-3 rounded-xl bg-[#0A2540] text-white font-bold text-sm hover:bg-[#0d2f4f] transition"
                  >
                    Close
                  </button>
                </div>
